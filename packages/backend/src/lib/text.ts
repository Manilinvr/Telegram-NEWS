...
/**
 * Убрать из текста источника то, чему не место в нашей публикации.
 *
 * Чужие посты заканчиваются призывами подписаться, ссылками на свой
 * канал, метками рекламы и приглашениями прислать новость. При разборе
 * по правилам (без модели) текст переносился дословно, и всё это
 * попадало в наш черновик — вплоть до «Подпишись на» с чужой ссылкой.
 *
 * Функция намеренно консервативна: удаляются только ссылки, упоминания
 * через @ и распространённые зазывы. Содержание новости не трогается —
 * сокращать и переписывать её должна модель, а не набор правил.
 */
export function stripPromotional(input: string): string {
  // Граница слова \b опирается на латиницу и с кириллицей не работает:
  // между пробелом и «н» границы нет, поэтому шаблоны с \b молча ничего
  // не находили. Конец слова задаётся явным «дальше не буква».
  const END = '(?![а-яёА-ЯЁ])';
  const TAIL = '[^.!?\\n]*';
  const PROMO = [
    `подпиш[иыа]\\S*\\s+на${END}${TAIL}`,
    `подписывайтесь${END}${TAIL}`,
    `подписаться${END}${TAIL}`,
    `наш[а-яё]*\\s+(?:канал|телеграм\\S*|тг)${END}${TAIL}`,
    `присыла[а-яё]*\\s+новост[а-яё]*${END}${TAIL}`,
    `прислать\\s+новост[а-яё]*${END}${TAIL}`,
    `слать\\s+новост[а-яё]*${END}${TAIL}`,
    `не\\s+груз[яи]тся\\s+фото\\s*\\/?\\s*видео\\??`,
    `реклама${END}${TAIL}`,
    `erid\\s*[:=]?\\s*\\S*`,
    `реп[оo]ст${END}${TAIL}`,
  ...
  // Ссылки любого вида, включая t.me и vk.com без схемы.
  text = text.replace(/https?:\/\/\S+/gu, ' ');
  text = text.replace(/\b(?:t\.me|telegram\.me|vk\.com|vk\.ru)\/\S+/giu, ' ');
  // Упоминания каналов через @.
  text = text.replace(/(^|[\s(])@[A-Za-z0-9_]{3,}/gu, '$1');

  for (const pattern of PROMO) text = text.replace(pattern, ' ');

  // Схлопываем пустоты, оставшиеся после вырезания.
  return text
    .replace(/[ \t ]+/gu, ' ')
    .replace(/\s+([,.!?;:])/gu, '$1')
    .replace(/\n{3,}/gu, '\n\n')
