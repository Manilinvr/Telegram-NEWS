...
Настройка `editorial` (редакционный стиль) принимает
`{ tone, summaryMaxSentences, useEmoji, signature, extraInstructions }`
и проверяется схемой: неизвестный тон и указания длиннее 2000 символов
отклоняются. Стиль влияет на тон и оформление, но не на правила работы с
фактами — см. [SETUP-AI.md](SETUP-AI.md).
